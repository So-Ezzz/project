...
def stft(audio_signal, fs=44100, window_size=1024, hop_size=512, batch_size=512):
    ...
    frames = frames[:, ::hop_size, :].copy()
    num_frames = frames.shape[1]
    frames = frames.reshape((-1, window_size)) * np.hamming(window_size)
...
# 主调度函数
def compute_stft(audio_signal, window_size=1024, hop_size=512, use_library=False):
    """
    根据选项使用 SciPy 或手动实现计算 STFT。
    """
    if use_library:
        return stft_lib(audio_signal, fs=44100, window_size=window_size, hop_size=hop_size)
    else:
        return stft(audio_signal, fs=44100, window_size=window_size, hop_size=hop_size)
