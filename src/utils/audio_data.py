...
class AudioData:
    ...
    # 计算所有音频的 STFT 特征
    def compute_stfts(self, train_data=True):
        """
        计算所有音频的 stft 特征。
        ...
        if train_data: # 训练数据
            self.stfts = stft(self.audios)
            save_pkl(self.stfts,pkl_path)
        else: # 从数据中加载
            self.stfts = load_pkl(pkl_path)

    # 计算所有音频的 mel 特征
    def compute_mels(self, num_bins=128, train_data=True):
        """
        计算所有音频的 mel 特征。
        ...
        if train_data: # 训练数据
            self.mels = mels(self.stfts, num_mel_bins=num_bins)
